...
if("string"==typeof s&&s.trim())try{const e=window.open("",s.trim());e&&e!==window&&!e.closed&&(d=e)}catch(e){}
...
if("*"!==c){const e=checkMixedContent(c);if(!e.safe)return Promise.reject(new Error(`[CrossMessage] ${e.reason}`))}return new Promise((r,i)=>{let s,a,w=!0;function u(){clearInterval(s),clearTimeout(a),window.removeEventListener("message",f),senders.delete(e),
// 内存保护：同时清理可能存在的接收方记录，防止极端场景下的内存泄漏
receivers.delete(e),w=!1}function f(n){const t=n.data;w&&t&&t.type===ACK_TYPE&&t.key===e&&n.source===d&&("*"!==c&&n.origin!==c||(r({key:e}),u()))}window.addEventListener("message",f),a=setTimeout(()=>{u(),i(new Error(`[CrossMessage] key="${e}" 等待回执超时（${o}ms）`))},o),s=setInterval(()=>{
// 检查目标窗口是否已关闭
